(function ()
{
    ...
    function ClearanceController($cookies, $rootScope, $log, $stateParams, $timeout, $filter, $sce, $mdDialog, msApi, ClearanceData, AlertService)
    {
        ...
        //$log.debug(ClearanceData.length + ' Clearances are found');
        //$log.debug('ClearanceData :' + JSON.stringify(ClearanceData));

        //-----------------------------Init the first tab
    	if($stateParams.tabIndex || ($stateParams.tabIndex === 0)) {
    		...
    		msApi.request('clearances@isCleared', {urlSuffix: 'cleared', jobInstanceId: $stateParams.clearanceId}, function(response) {
    			...
    			if(($stateParams.tabIndex < 4 ) || (($stateParams.tabIndex === 4) && !vm.cleared)) {
